"""
train_seg.py — Training entrypoint for UNet++ damage segmentation.

Usage:
    PYTHONPATH=. python src/train/train_seg.py --config configs/train_seg.yaml --paths-cfg configs/paths.yaml
    PYTHONPATH=. python src/train/train_seg.py --config configs/train_seg.yaml --paths-cfg configs/paths.yaml --fast-dev-run
"""
...
def _build_callbacks(cfg: dict, paths_cfg: Path) -> list:
    ...
    return [ckpt_cb, LearningRateMonitor(logging_interval="epoch"), RichProgressBar()]


def main() -> None:
    parser = argparse.ArgumentParser(description="Train UNet++ damage segmentor.")
